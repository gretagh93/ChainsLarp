...
import com.chainslarp.app.application.network.firebase.SnapshotListenerEvent.*
 ...
 */
data class FirestoreResultDocument<T>(
    val model: T,
    ...
    companion object {
        ...
        inline fun <reified FirebaseModel, Model> fromSnapshot(
            ...
            documentMapping: (DocumentSnapshot) -> FirebaseModel?
        ): FirestoreResultDocument<Model> {

            if (!documentSnapshot.exists()) throw IllegalArgumentException("Document is empty")
...
sealed class SnapshotListenerEvent<Model> {
    ...
    companion object {
        ...
        inline fun <reified FirebaseModel, Model> fromSnapshot(
            ...
            mappingFn: ModelMapping<FirebaseModel, Model>,
            documentMapping: (DocumentSnapshot) -> FirebaseModel?
        ): FirestoreDocument<Model> =
            FirestoreDocument(
                FirestoreResultDocument.fromSnapshot(
                    documentSnapshot,
                    mappingFn,
                    documentMapping
                )
        ...
        inline fun <reified FirebaseModel, Model> fromSnapshot(
            ...
            mappingFn: ModelMapping<FirebaseModel, Model>,
            documentMapping: (DocumentSnapshot) -> FirebaseModel?
        ): FirestoreDocumentChanges<Model> {

            if (querySnapshot.isEmpty) throw IllegalArgumentException("Document is empty")
