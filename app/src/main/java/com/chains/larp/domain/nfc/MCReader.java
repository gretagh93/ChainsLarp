...
public class MCReader {
    ...
    public SparseArray<String[]> readAsMuchAsPossible() {
        ...
        setMappingRange(0,  getSectorCount());
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(ByteUtils.Companion.byte2Hex(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (noData) {
                ...
                if (!useAsKeyB) {
                    if (isKeyBReadable(ByteUtils.Companion.hex2ByteArray(
                            ret[last].substring(12, 20)))) {
                        ret[last] = ByteUtils.Companion.byte2Hex(key)
                    ...
                    ret[last] = NO_KEY + ret[last].substring(12, 20)
                            + ByteUtils.Companion.byte2Hex(key);
                }
            }
        }
        return ret;
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            if (!error && (foundKeys[0] || foundKeys[1])) {
                ...
                byte[] fKey = ByteUtils.Companion.hex2ByteArray("FFFFFFFFFFFF");
    ...
    /**
     * If you full key map, you have to call {@link #buildNextKeyMapPart()} as
